...
app.post('/api/rounds', requireAuth, async (req, res) => {
  const { title, emoji, category, blurb, lat, lng, place, photo, link, event_at, requires_approval, recurrence } = req.body || {};
  ...
  await db.prepare(`INSERT INTO rounds (id,title,emoji,category,blurb,host_id,created_at,lat,lng,place,photo,link,event_at,requires_approval,recurrence)
              VALUES (@id,@title,@emoji,@category,@blurb,@host_id,@created_at,@lat,@lng,@place,@photo,@link,@event_at,@requires_approval,@recurrence)`).run(round);
...
function normalizeSeatGeekEvent(e, label) {
  const v = e.venue || {};
  const typeMap = { concert: ['Music', '🎵'], music_festival: ['Music', '🎵'], sports: ['Sports', '🏃'], theater: ['Theater', '🎭'], comedy: ['Comedy', '🎤'] };
  ...
  return {
    ...
    category, emoji,
    photo: (e.performers && e.performers[0] && (e.performers[0].image || (e.performers[0].images && e.performers[0].images.huge))) || null,
    link: e.url || null,
    place: v.name || v.city || '',
    city: v.city || '',
    state: (v.state || '') ,
...
async function fetchEventbrite(token, label) {
  ...
  const events = []; let lastError = null;
  try {
    // Which organizations can this token see?
    const orgRes = await fetch('https://www.eventbriteapi.com/v3/users/me/organizations/', H);
    if (!orgRes.ok) {
      let d = orgRes.statusText; try { const b = await orgRes.json(); d = b.error_description || d; } catch (e) {}
      return { events: [], error: 'Eventbrite returned HTTP ' + orgRes.status + (d ? ' — ' + d : '') };
    }
    const orgs = ((await orgRes.json()).organizations) || [];
    for (const org of orgs.slice(0, 5)) {
      let url = 'https://www.eventbriteapi.com/v3/organizations/' + org.id + '/events/?status=live&order_by=start_asc&expand=venue&page_size=200';
      let guard = 0;
      while (url && guard < 10) {
        const r = await fetch(url, H);
        if (!r.ok) { lastError = 'Eventbrite events HTTP ' + r.status; break; }
        const data = await r.json();
        for (const e of (data.events || [])) events.push(normalizeEventbriteEvent(e, label));
        url = (data.pagination && data.pagination.has_more_items && data.pagination.continuation)
          ? ('https://www.eventbriteapi.com/v3/organizations/' + org.id + '/events/?status=live&order_by=start_asc&expand=venue&page_size=200&continuation=' + data.pagination.continuation)
          : null;
        guard++;
        await new Promise(res => setTimeout(res, 200));
      }
