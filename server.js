...
const EVENT_PROVIDERS = {
  ...
  // Paste-a-key providers. Each takes the API key the manager enters and returns
  // normalized events. See fetchSeatGeek / fetchEventbrite below.
...
async function fetchWebsiteEvents(s) {
  ...
  if (!deduped.length) {
    return { events: [], error: "Couldn't read events from that site automatically. ShowUpp reads schema.org event data, The Events Calendar (WordPress), and iCal/.ics feeds — most event sites use one of these. If this one doesn't, paste the direct /events page URL, or add this source's events by hand." };
  }
