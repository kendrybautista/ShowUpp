...
async function fetchWebsiteEvents(s) {
  ...
  // F) JavaScript-rendered sites (SPAs). The plain fetch above only sees the initial HTML,
  // so sites that build their event list with JavaScript after load return nothing (this
  // is the sdconcerts.com case). As a last resort we fetch the page through Jina Reader,
  // which runs a headless browser to execute the JS, and ask it for the FULLY-RENDERED
  // HTML — then run our normal JSON-LD + HTML parsers on that. Only fires when everything
  // else came up empty, so it's rare and cheap. Set JINA_READER_KEY for higher rate limits.
  if (!out.length) {
    for (const pg of [pageUrl, origin + '/events', origin + '/calendar', origin + '/shows'].slice(0, 4)) {
      try {
        const readerUrl = 'https://r.jina.ai/' + pg;
        const headers = { 'X-Respond-With': 'html', 'X-Timeout': '20', 'Accept': 'text/html' };
        if (process.env.JINA_READER_KEY) headers['Authorization'] = 'Bearer ' + process.env.JINA_READER_KEY;
        const rr = await fetch(readerUrl, { headers });
        if (!rr.ok) continue;
        const html = await rr.text();
        if (!html) continue;
        // Try structured data first (the rendered page often now HAS JSON-LD), then HTML.
