...
app.get('/api/events/locations', requireAuth, async (req, res) => {
  ...
  try {
    // Cities for a chosen state (optionally within a country).
    if (stateQ) {
    ...
    // States/regions — scoped to a country when one is chosen (this is what keeps the
    // list short and relevant per the redesign).
    if (countryQ) {
      const rows = await db.prepare(`
    ...
    // No country chosen → no state list (the redesign keeps state empty until a country
    // is picked). Return empty so the dropdown stays blank.
    res.json({ states: [] });
