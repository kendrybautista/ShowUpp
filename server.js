...
app.post('/api/admin/user/:id/update', requireAuth, requireAdmin, async (req, res) => {
  ...
  const { name, username, email, city, phone, bio } = req.body || {};
  // Validate + normalize username/email, and enforce uniqueness against OTHER users.
  let cleanUser = null;
  ...
  await db.prepare(`UPDATE users SET
      ...
      bio = COALESCE(?, bio)
    WHERE id = ?`).run(
    (name !== undefined && String(name).trim()) ? String(name).trim().slice(0, 80) : null,
    cleanUser,
    cleanEmail,
    (city !== undefined) ? String(city).trim().slice(0, 120) : null,
