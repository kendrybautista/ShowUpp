...
async function fetchWebsiteEvents(s) {
  ...
  if (!deduped.length) {
    return { events: [], error: "Couldn't read events from that page. ShowUpp tries schema.org data, The Events Calendar, iCal feeds, and reads the visible event list directly — but some sites build their events with JavaScript after load, which can't be read server-side. Try pasting the direct /events page URL, or add this source's events by hand." };
  }
